...
class TooManyRequestsRetryMiddleware(RetryMiddleware):
    def process_response(self, request, response, spider):
        ...
        if response.status == 429:
            ...
            # Проверяем, достигли ли максимального количества повторных попыток
            if retries >= self.max_retry_times:
                spider.logger.error(f"Превышено количество попыток для {response.url}. Завершаем запрос.")
                return response  # Если попытки исчерпаны, возвращаем оригинальный ответ
    ...
    def process_exception(self, request, exception, spider):
        spider.logger.error(f"Произошла ошибка при обработке {request.url}: {exception}")
...
class FreeproxySpiderSpiderMiddleware:
...
class FreeproxySpiderDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
